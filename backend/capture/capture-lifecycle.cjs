...
function selectCommentMetricUpdates({ rows, seenKeys, metrics, keyOf }) {
  ...
  for (const row of rows || []) {
    ...
    const next = {
      likes: known(row.likes) ?? previous?.likes ?? null,
      replyCount: known(row.replyCount) ?? previous?.replyCount ?? null,
    };
    if (!previous || next.likes !== previous.likes || next.replyCount !== previous.replyCount) {
      metrics.set(key, next);
      updates.set(key, { ...row, ...next });
    }
  }
