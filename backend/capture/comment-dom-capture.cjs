const COMMENT_DOM_CAPTURE_SCRIPT = String.raw`(() => {
  ...
  return candidates.slice(0, 2000).map((element) => {
    ...
    const ownMetric = (attribute, selectors) => {
      const direct = element.getAttribute(attribute);
      if (direct != null) return direct;
      const target = Array.from(element.querySelectorAll(selectors)).find((candidate) => {
        for (let parent = candidate.parentElement; parent && parent !== element; parent = parent.parentElement) {
          if (seen.has(parent)) return false;
        }
        return true;
      });
      const text = textOf(target).replace(/^(?:点赞|赞|回复)\s*/, '').replace(/\s*(?:次点赞|赞|条回复)$/, '');
      return /^\d[\d,]*(?:\.\d+)?(?:万|[wk])?\+?$/i.test(text) ? text : null;
    };
    const likes = ownMetric('data-like-count', '.like .count,.like-wrapper .count,.like-count,[class*="likeCount"]');
    const replyCount = ownMetric('data-reply-count', '.reply .count,.reply-count,[class*="replyCount"]');
    return { id, nickname, authorId, content, time, region, likes, replyCount };
