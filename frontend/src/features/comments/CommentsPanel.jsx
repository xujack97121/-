...
function CommentDetails({ accountId, group, filters, onClose, onStart, onStop, collecting, starting, busy, onExport, onConfigure, active, settingsRevision }) {
  ...
  return <aside className="comment-details" aria-label="笔记评论统计分析" onKeyDown={(event) => {
    ...
    <div className="comment-details-scroll">
      ...
      <section className="comment-detail-list">
        <div className="comment-section-heading"><h3>评论明细 <span>{rows.length} / {group.comments.length}</span></h3>
          <button className="icon-button" type="button" title="导出当前笔记筛选结果" aria-label="导出当前笔记评论" disabled={!rows.length} onClick={() => onExport(rows, group.title)}><IconDownload size={16} /></button>
        </div>
    ...
    <footer className="comment-details-actions">
      ...
      {collecting ? <button className="button" type="button" onClick={onStop}><IconPlayerStop size={15} />停止采集</button>
        : <button className="button primary" type="button" disabled={starting || busy || !group.link} title="重新打开笔记采集，保留已有评论并去重" onClick={() => onStart(group)}><IconRefresh size={15} />{starting ? "正在打开" : group.comments.length ? "重新采集" : "采集评论"}</button>}
    </footer>
  </aside>;
}

export function CommentsPanel({ accountId, accountName, data, state, setState, tasks, onStart, onStop, onOpen, activeNoteId, starting, notify, downloadCsv, onConfigure, active, settingsRevision }) {
  const [deleting, setDeleting] = useState(false);
  const scope = state.scope || "all";
  const openerRef = useRef(null);
  const listRef = useRef(null);
  const groups = useMemo(() => groupCommentNotes(data.notes, data.comments, tasks), [data.notes, data.comments, tasks]);
  ...
  const exportRows = (items, title = "笔记评论") => downloadCsv(`${accountName}-${title.replace(/[\\/:*?"<>|]/g, "_").slice(0, 70)}.csv`, items, columns);
  const remove = async (group = null) => {
    ...
    if (!window.confirm(`${message}\n不会删除小红书原笔记或平台评论。此操作无法撤销。`)) return;
    setDeleting(true);
    try {
  ...
  return <div className="comments-panel">
    <div className="comment-controls">
      <form className="comment-add-form" onSubmit={(event) => { event.preventDefault(); onStart({ link: state.url, title: "手动添加的笔记" }); }}>
        <label htmlFor="comment-note-link">评论采集</label>
        <input id="comment-note-link" aria-label="评论采集笔记链接" value={state.url || ""} placeholder="小红书笔记链接" onChange={(event) => setState({ url: event.target.value })} />
      ...
      <details className="comment-filter-controls">
        ...
        <div className="comment-filters">
          ...
          <label><input type="checkbox" checked={Boolean(state.unique)} onChange={(event) => setState({ unique: event.target.checked })} />按笔记去重用户</label>
      ...
      <div className="comment-library-toolbar">
        <div className="comment-scopes" role="group" aria-label="笔记范围">{[
        ...
        <button className="icon-button" type="button" aria-label="导出全部笔记评论" title={`导出筛选后的 ${rows.length} 条评论`} disabled={!rows.length} onClick={() => exportRows(rows)}><IconDownload size={16} /></button>
    ...
    <div className={`comment-library ${selected ? "has-details" : ""}`}>
      <div ref={listRef} tabIndex={-1} className="comment-card-scroll" aria-label="笔记评论卡片">
        <div className="comment-note-grid">{visible.map((group) => {
          ...
          return <article key={group.id} className={`comment-note-card ${selected?.id === group.id ? "is-selected" : ""}`}>
            ...
            <footer className="comment-card-actions">
              ...
              <button className="button quiet compact" type="button" title="重新打开笔记采集，保留已有评论并去重" disabled={collecting || Boolean(starting) || deleting || !group.link} onClick={() => onStart(group)}><IconRefresh size={14} />{starting === group.id ? "正在打开" : collecting ? "采集中" : group.comments.length ? "重新采集" : "采集评论"}</button>
            </footer>
          </article>;
        })}</div>
        {!visible.length && <div className="comment-empty"><IconMessageCircle size={28} /><strong>{scope === "collected" ? "暂无已采集的笔记" : scope === "pending" ? "没有待采集笔记" : "暂无笔记评论"}</strong></div>}
      </div>
