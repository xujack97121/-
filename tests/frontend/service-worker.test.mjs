...
await import(`../../frontend/public/service-worker.js?test=${Date.now()}`);
...
assert.equal(state.collectorComments[0].likes, null);
assert.equal(state.collectorComments[0].replyCount, null);
await deliver("https://www.xiaohongshu.com/api/sns/web/v2/comment/page?note_id=note-001", {
  comments: [{ id: "comment-001", content: "示例公开评论", user: { nickname: "示例用户", user_id: "987654321000" }, like_count: "1.5万", sub_comment_count: 8 }],
});
assert.equal(state.collectorComments[0].likes, 15000);
assert.equal(state.collectorComments[0].replyCount, 8);
await deliver("https://www.xiaohongshu.com/api/sns/web/v2/comment/page?note_id=note-001", {
  comments: [{ id: "comment-001", content: "示例公开评论", user: { nickname: "示例用户", user_id: "987654321000" } }],
});
assert.equal(state.collectorComments[0].likes, 15000, "Missing measurements preserve earlier metrics");
assert.equal(state.collectorComments[0].replyCount, 8);
